...
import tooltip from "./tooltip.js";
  ...
  const days = ["Mo", "Tu", "We", "Th", "Fr", "Sa", "Su"],
    ...
    legendElementWidth = gridSize.width * 2,
    buckets = 7,
    colors = [
      "#ffffcc",
      "#c7e9b4",
...
<!-- legend -->
<g id="legend" transform="translate({gridSize.width}, 280)">
{#each scale as q,i}
<g transform="translate({legendElementWidth*i}, 0)">
<rect fill={colorScale(q)} width={legendElementWidth} height={gridSize.height/2}/>
<text text-anchor="middle" y={gridSize.height} x={gridSize.width}>≥ {Math.floor(q)}</text>
</g>
{/each}
